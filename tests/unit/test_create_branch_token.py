...
class TestCreateBranchTokenIntegration:
    ...
    def test_token_provider_called_on_branch_creation(
        ...
        mock_github_client = GitHubClient(token_provider=mock_token_provider)
    ...
    def test_token_used_in_github_api_requests(
        ...
        test_token = "ghs_test_installation_token"
        ...
        with patch("httpx.Client") as mock_httpx:
            ...
            mock_post_response = MagicMock()
            mock_post_response.status_code = 201
            mock_post_response.json.return_value = {"ref": "refs/heads/feature/test"}
            ...
            with patch.object(_app_state, "github_client", mock_github_client):
                ...
                # Verify the token was used in API calls
                get_call = mock_http_client.get.call_args
                post_call = mock_http_client.post.call_args

                get_headers = get_call[1]["headers"]
                post_headers = post_call[1]["headers"]

                assert get_headers["Authorization"] == f"Bearer {test_token}"
                assert post_headers["Authorization"] == f"Bearer {test_token}"
    ...
    def test_fresh_token_per_request(
        ...
        mock_token_provider.get_installation_token.side_effect = [
            "ghs_token_1",
            "ghs_token_2",
        ]
        ...
        with patch("httpx.Client") as mock_httpx:
            ...
            with patch.object(_app_state, "github_client", mock_github_client):
                ...
                # Token provider should be called twice (once per request)
                assert mock_token_provider.get_installation_token.call_count == 2


class TestGitHubClientTokenUsage:
    """Tests for GitHubClient token usage."""
