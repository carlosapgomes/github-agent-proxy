...
DEFAULT_COMMANDS = {
    "python": {
        ...
        "format": "black --check .",
        "security": "bandit -r . -q",
        "django_check": "python3 manage.py check"
    ...
    try:
        ...
        for raw_line in section_content.split("\n"):
            ...
            if stripped.startswith("###"):
                current_label = (
                    re.sub(r"^#+\s*", "", stripped).strip().lower() or "command"
                )
                continue
    ...
    if format == "json":
        return json.dumps(
            {
                ...
                "summary": {
                    "total": len(results),
                    "passed": sum(
                        1 for r in results if r["success"] and not r["skipped"]
                    ),
                    "failed": sum(
                        1 for r in results if not r["success"] and not r["skipped"]
                    ),
        ...
        for result in results:
            if result["skipped"]:
                ...
                report.append(
                    f"[FAIL] {result['name']} ({result['duration']}s): {result['error']}"
                )
                if result["output"]:
                    lines = result["output"].split("\n")
                    if len(lines) > 5:
...
Exemplos:
  ...
  %(prog)s --list             # Lista comandos que seriam executados
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Modo verboso (mostra mais detalhes)"
    )
    ...
    if not commands:
        ...
        print(
            "   Crie um AGENTS.md ou use uma stack suportada (Python/JavaScript).",
            file=sys.stderr,
        )
        return 1
    ...
    results = []
    print(
        f"🚀 Executando quality gate com {len(commands)} comandos...", file=sys.stderr
    )
