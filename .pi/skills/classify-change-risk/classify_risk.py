...
WEIGHT_FACTORS = {
    "impact_users": {"all": 3.0, "many": 2.0, "some": 1.5, "few": 1.0, "none": 0.5},
    "impact_data": {"sensitive": 3.0, "important": 2.0, "regular": 1.0, "none": 0.5},
    "complexity": {
        "very_high": 2.5,
        "high": 2.0,
        "medium": 1.5,
    ...
    "dependencies": {"many": 2.0, "some": 1.5, "few": 1.2, "none": 1.0},
}

# Limiares para classificação
THRESHOLDS = {"critical": 8.0, "professional": 4.0, "essential": 0.0}
...
def analyze_text(text: str) -> Dict[str, Any]:
    ...
    patterns = {
        "has_security_terms": bool(
            re.search(r"\b(security|auth|password|encrypt)\b", text_lower)
        ),
        "has_data_terms": bool(
            re.search(r"\b(data|database|gdpr|lgpd|pii)\b", text_lower)
        ),
        "has_financial_terms": bool(
            re.search(r"\b(payment|money|financial|tax)\b", text_lower)
        ),
        "has_infrastructure_terms": bool(
            re.search(r"\b(server|deploy|production|infra)\b", text_lower)
        ),
        "has_test_terms": bool(
            re.search(r"\b(test|coverage|unit|integration)\b", text_lower)
        ),
        "has_documentation_terms": bool(
            re.search(r"\b(doc|readme|comment|guide)\b", text_lower)
        ),
...
def calculate_risk_score(
    analysis: Dict[str, Any], factors: Dict[str, str]
) -> Dict[str, Any]:
    ...
    impact_users = WEIGHT_FACTORS["impact_users"].get(
        factors.get("impact_users", "some"), 1.0
    )
    impact_data = WEIGHT_FACTORS["impact_data"].get(
        factors.get("impact_data", "regular"), 1.0
    )
    complexity = WEIGHT_FACTORS["complexity"].get(
        factors.get("complexity", "medium"), 1.0
    )
    dependencies = WEIGHT_FACTORS["dependencies"].get(
        factors.get("dependencies", "few"), 1.0
    )
...
def get_recommendations(
    ...
    if level == "CRITICAL":
        ...
        if analysis["patterns"]["has_security_terms"]:
            recommendations.append(
                "🔒 **Segurança**: Realizar análise de segurança específica (bandit, safety check)"
            )
        if analysis["patterns"]["has_data_terms"]:
            recommendations.append(
                "💾 **Dados**: Backup obrigatório antes da mudança, validação de integridade"
            )
        if analysis["patterns"]["has_financial_terms"]:
            recommendations.append(
                "💰 **Financeiro**: Testes com dados de teste, validação com contabilidade"
            )
        ...
        if analysis["patterns"]["has_test_terms"]:
            recommendations.append(
                "🧪 **Testes**: Garantir cobertura adequada para novas funcionalidades"
            )
        if analysis["patterns"]["has_ui_terms"]:
            recommendations.append(
                "🎨 **UI/UX**: Testar em diferentes dispositivos e navegadores"
            )
    ...
    recommendations.extend(
        [
            ...
            "⏱️  **Estimativa**: Ajustar estimativa de tempo baseado no nível de risco",
            "🔍 **Monitoramento**: Monitorar após deploy conforme nível de risco",
        ]
    )
...
def read_project_context() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        context = {
            "has_sensitive_data": "dados sensíveis" in content.lower()
            or "lgpd" in content.lower()
            or "gdpr" in content.lower(),
            "has_financial": "financeiro" in content.lower()
            or "pagamento" in content.lower()
            or "payment" in content.lower(),
            "has_infrastructure": "infraestrutura" in content.lower()
            or "production" in content.lower(),
            "is_public_sector": "público" in content.lower()
            or "governo" in content.lower()
            or "government" in content.lower(),
        }
...
def adjust_for_project_context(
    ...
    if score_result["score"] >= THRESHOLDS["critical"] and old_level != "CRITICAL":
        ...
        score_result["score"] >= THRESHOLDS["professional"] and old_level == "ESSENTIAL"
    ):
...
def classify_change(
    description: str, factors: Optional[Dict[str, str]] = None
) -> Dict[str, Any]:
    ...
    if score_result["level"] == "ESSENTIAL" and not simple_bugfix:
        ...
        score_result["weighted_score"] = max(
            score_result.get("weighted_score", 0.0), THRESHOLDS["professional"]
        )
    ...
    artifact_policy = build_artifact_policy(
        score_result["level"], simple_bugfix=simple_bugfix
    )
    ...
    return {
        ...
        "analysis": {
            ...
            "keyword_matches": {
                k: v[:5] for k, v in analysis["keyword_matches"].items() if v
            },  # Top 5
...
Fatores de ponderação:
  ...
  dependencies: none, few, some, many
    ...
    parser.add_argument(
        "description", nargs="?", help="Descrição da mudança a ser classificada"
    )

    parser.add_argument("--file", "-f", help="Arquivo contendo a descrição da mudança")
    ...
    try:
        ...
        if result["risk_assessment"]["level"] == "CRITICAL":
            ...
            result["risk_assessment"]["level"] == "PROFESSIONAL"
            and args.strict_exit_codes
        ):
...
def generate_text_report(result: Dict[str, Any], verbose: bool = False) -> str:
    ...
    if policy:
        design_flag = (
            "obrigatorio" if policy.get("design_required") else "opcional (excecao)"
        )
        lines.append(
            f"📁 Artefatos: modo {policy.get('mode', 'N/A')} | design.md {design_flag}"
        )
    ...
    if len(result["recommendations"]) > 10:
        lines.append(
            f"  ... e mais {len(result['recommendations']) - 10} recomendações"
        )
...
def generate_markdown_report(result: Dict[str, Any], verbose: bool = False) -> str:
    ...
    if policy:
        ...
        lines.append(
            f"- **design.md:** {'obrigatório' if policy.get('design_required') else 'opcional (exceção)'}"
        )
        if policy.get("reason"):
            lines.append(f"- **Motivo:** {policy['reason']}")
    lines.append("")
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Classificado por ESAA Risk Classifier v1.0*")
